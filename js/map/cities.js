...
data.forEach(res =>{
    res.children.forEach(item=>{
        arr.push(`${item.name}:[${item.log},${item.lat}]`)
        // console.log(`${item.name}:[${item.log},${item.lat}],`);
    })
})
// 写入文件内容（如果文件不存在会创建一个文件）
// 传递了追加参数 { 'flag': 'a' }
fs.writeFile('../../assets/data/cities.txt', arr.toString(), { 'flag': 'a' }, function(err) {
    if (err) {
        throw err;
    }
    ...
    // 写入成功后读取测试
    fs.readFile('../../assets/data/cities.txt', 'utf-8', function(err, data) {
        if (err) {
            throw err;
        }
        console.log(data);
